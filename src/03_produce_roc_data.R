...
all_var_set_combinations<-generate_var_set_combinations(main_var_sets,sub_var_sets,missing_var_sets)
colnames(all_var_set_combinations)<-c('main_var_set','sub_var_set','missing_var_set')

if (!with_panelapp){
  all_var_set_combinations<-all_var_set_combinations%>%filter(!grepl('panelapp',sub_var_set))
}
...
save(procs,roc_metrics_table,file=glue('{data_folder_name}/roc_data.RData'))
