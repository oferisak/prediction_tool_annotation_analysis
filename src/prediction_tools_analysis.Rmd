...
top_tools <- overall_performance_table %>%
  group_by(complete) %>%
...
roc_table <- raw_roc_table %>%
  ...
  mutate(
    tool_name = ifelse(is.na(is_top), "Other", tool),
...
ggsave(
  filename = glue("{analysis_folder_name}/top_models_roc_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
top_performing_tools_original_names <- top_tools_rank %>%
  pull(tool) %>%
  unique()

# info on top performing tools for the manuscript
for_text <- overall_performance_table %>%
  filter(filter1 == "at_or_after_2023", grepl("bayes|metarnn|clinpred", tool), complete == "complete") %>%
  select(contains(c("tool", "AUC", "precision90")))
...
overall_score_thresholds_plot <-
  proc_data %>%
  filter(var_sets[[year_to_analyze]]) %>%
...
overall_score_thresholds_plot
ggsave(
  filename = glue("{analysis_folder_name}/overall_score_thresholds_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
missing_count <- sapply(tools_without_excluded, function(tool) {
  proc_data %>%
    filter(is.na(!!sym(tool))) %>%
    nrow()
})
missing_count <- missing_count %>%
  as.data.frame() %>%
  rename(n_missing = ".") %>%
  mutate(missing_rate = n_missing / nrow(proc_data))
missing_count
...
missing_rate_plot
ggsave(
  filename = glue("{analysis_folder_name}/01_missing_rate_plot.{Sys.Date()}.png"),
...
for (year in as.character(2013:2024)) {
  ...
  year_missing_count <- sapply(tools_without_excluded, function(tool) {
    year_data %>%
      filter(is.na(!!sym(tool))) %>%
      nrow()
  })
...
perf_by_year_table <- perf_by_year_table %>%
  ...
  mutate(
    pub_year = as.character(pub_year),
...
perf_by_year_all_plot <-
  ...
  geom_vline(aes(xintercept = training_year, color = "Tool creation year (For tools that utillized ClinVar during training)"), data = perf_by_year_table, linetype = 2) +
  theme_minimal() +
...
ggsave(
  filename = glue("{analysis_folder_name}/all_tools_{perf_metric}_by_year_plot.{Sys.Date()}.png"),
...
ggsave(
  filename = glue("{analysis_folder_name}/top_tools_{perf_metric}_by_year_plot.{Sys.Date()}.png"),
...
year_analysis_table <- perf_by_year_table %>%
  mutate(
...
no_clinvar_training_tools <- year_analysis_table %>%
  group_by(tool) %>%
  summarize(before_and_after = length(unique(before_training))) %>%
  filter(before_and_after < 2) %>%
  pull(tool)
...
ggsave(
  filename = glue("{analysis_folder_name}/overall_with_by_year.combined_plot.{perf_metric}.{Sys.Date()}.png"),
...
performance_by_conservation_table <-
  ...
  mutate(tool = stringr::str_replace(tool, "_score.+", "") %>% toupper()) %>%
  mutate(
    tool = forcats::fct_reorder(tool, AUC50.),
...
cons_overall_performance <-
  ...
  filter(name %in% c("AUC", "Sensitivity", "Specificity")) %>%
  select(-c(total, conservation_level)) %>%
  distinct()
...
ggsave(
  filename = glue("{analysis_folder_name}/performance_by_conservation_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
conservation_score_thresholds_plot <-
  ...
  mutate(tool = stringr::str_replace(tool, "_score.+", "") %>% toupper()) %>%
  ggplot(aes(
    x = factor(filter2, levels = rev(levels(factor(filter2)))),
...
conservation_score_thresholds_plot
ggsave(
  filename = glue("{analysis_folder_name}/conservation_score_thresholds_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
ggsave(
  filename = glue("{analysis_folder_name}/conservation.combined_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
revel_score_distribution_example_plot
ggsave(
  filename = glue("{analysis_folder_name}/revel_score_distribution_example_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
top_tools_conservation <- performance_by_conservation_table_complete %>%
  ...
  mutate(
    rank = row_number(), # Assign ranks within each conservation level
    conservation_level = factor(conservation_level, levels = c("Low", "Intermediate", "High")),
...
ggsave(
  filename = glue("{analysis_folder_name}/performance_by_conservation_bump_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
moi_analysis_res <- calculate_roc_metrics(proc_data, analysis_var_sets, tools_to_test,
  complete = F,
...
performance_by_moi_plot <-
  ...
  coord_flip() +
  labs(y = "AUC difference", fill = NULL, x = NULL) +
  guides(color = "none") +
  theme_minimal() +
  theme(
...
ggsave(
  filename = glue("{analysis_folder_name}/performance_by_moi_plot.{Sys.Date()}.png"),
...
moi_score_thresholds_plot
ggsave(
  filename = glue("{analysis_folder_name}/moi_score_thresholds_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
ggsave(
  filename = glue("{analysis_folder_name}/moi.combined_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
mismatchoe_vars <- c(
  grep("mismatch_min_oe_ratio:", names(var_sets), value = T),
...
performance_by_mismatchoe_plot <-
  performance_by_mismatchoe_table %>%
  filter(!(grepl("regeneron", mismatch_oe))) %>%
  mutate(mismatch_oe = as.numeric(mismatch_oe)) %>%
  filter(tool %in% top_performing_tools) %>%
...
ggsave(
  filename = glue("{analysis_folder_name}/performance_by_mismatch_oe_plot.{Sys.Date()}.png"),
...
mismatchoe_score_thresholds_plot <-
  ...
  mutate(
    tool = stringr::str_replace(tool, "_score.+", "") %>% toupper(),
...
mismatchoe_score_thresholds_plot
ggsave(
  filename = glue("{analysis_folder_name}/mismatchoe_score_thresholds_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
ggsave(
  filename = glue("{analysis_folder_name}/conservation_mismatchoe.combined_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
af_vals <- proc_data %>%
  mutate(af = af_cats) %>%
  select(af, all_of(c(tools_to_test, af_col))) %>%
  pivot_longer(-c(af, af_col)) %>%
  filter(!is.na(value)) %>%
  group_by(af, name) %>%
  summarize(
    mean_af = mean(!!sym(af_col)),
    median_af = median(!!sym(af_col))
  ) %>%
  ungroup() %>%
  mutate(
    mean_af = ifelse(af == "0", 0, mean_af),
    median_af = ifelse(af == "0", 0, median_af)
  ) %>%
  rename(tool = name)
...
# all tools
af_model_all <- lme(AUC50. ~ af, random = ~ 1 | tool, data = performance_by_af_table)
summary(af_model_all)

# top performing tools
af_model_top_tools <- lme(AUC50. ~ af, random = ~ 1 | tool, data = performance_by_af_table %>% filter(tool %in% top_performing_tools))
summary(af_model_top_tools)

write.table(
  data.frame(group = "all_tools", summary(af_model_all)$tTable) %>%
    bind_rows(data.frame(group = "top_tools", summary(af_model_top_tools)$tTable)),
  file = glue("{analysis_folder_name}/af_mixed_effects_model_{time_var_set}_table.{Sys.Date()}.csv"), quote = FALSE, sep = "\t"
)
...
ggsave(
  filename = glue("{analysis_folder_name}/performance_by_af_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
af_score_thresholds_plot <-
  ...
  facet_wrap(tool ~ ., scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "top", axis.text.x = element_text(angle = -60, hjust = 0)) +
  labs(x = "Conservation", y = "Score", color = "Criteria", size = "TPR")
af_score_thresholds_plot

ggsave(
  filename = glue("{analysis_folder_name}/af_score_thresholds_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
ggsave(
  filename = glue("{analysis_folder_name}/af.combined_plot.{year_to_analyze}.{Sys.Date()}.png"),
...
for (pa_col in panelapp_cols) {
  ...
  for (tool in tools_to_test) {
    min_count <- proc_data %>%
      filter(var_sets[[year_to_analyze]], !is.na(!!sym(tool))) %>%
      count(!!sym(pa_col), clinvar_class) %>%
      filter(!is.na(!!sym(pa_col))) %>%
      pull(n) %>%
      min()
...
panelapp_analysis_res <- calculate_roc_metrics(proc_data, analysis_var_sets, tools_to_test,
  complete = F, save_rocs = F,
...
pa_comp_res <-
  ...
  mutate(
    fill_color = ifelse(p.value < 0.05 & abs(estimate) > 0.01, fill_value, 0),
...
panelapp_performance_plot_per_panel <-
  ...
  labs(x = NULL, y = "AUCROC difference") +
  guides(fill = "none") +
  annotate("segment",
    y = 0.002, yend = 0.01, x = -Inf, xend = -Inf,
...
ggsave(
  filename = glue("{analysis_folder_name}/panelapp_performance_plot_per_panel.{year_to_analyze}.{Sys.Date()}.png"),
...
feature_comp <-
  pa_comp_res %>%
  select(panelapp, p.value, estimate) %>%
  distinct() %>%
  mutate(
    sig_better = ifelse(p.value < 0.05 & estimate > 0.01, 1, 0),
  ...
  left_join(panelapp_analysis_res$roc_metrics_table %>% select(panelapp_original = filter2) %>% distinct() %>%
    ...
    mutate(panelapp = stringr::str_replace_all(panelapp, "\\.", " ")))
better_panels <- feature_comp %>%
  filter(sig_better == 1) %>%
  pull(panelapp_original)
worse_panels <- feature_comp %>%
  filter(sig_worse == 1) %>%
  pull(panelapp_original)
...
pa_attributes <- proc_data %>%
  ...
  mutate(
    better_panel = as.integer(Reduce(`|`, var_sets[names(var_sets) %in% better_panels])),
...
pa_attributes %>%
  count(perf, clinvar_class, Conservation) %>%
  group_by(perf, clinvar_class) %>%
  mutate(rate = n / sum(n))

pa_attributes %>%
  filter(Conservation == "Low", perf %in% c("Better AUROC", "Same", "Worse AUROC")) %>%
...
pa_attributes %>%
  filter(MOI == "XL", perf %in% c("Better AUROC", "Same", "Worse AUROC")) %>%
  count(perf, clinvar_class) %>%
  group_by(perf) %>%
  mutate(rate = n / sum(n))

pa_attributes %>%
  filter(Conservation == "Low", perf %in% c("Better AUROC", "Worse AUROC")) %>%
...
panel_table <- proc_data %>%
  select(selected_panel, missense_min_oe_ratio, moi_gencc) %>%
  mutate(
    conservation = ifelse(var_sets$high_conservation, "High",
...
pa_per_tool_attribute_effect %>%
  select(panelapp, tool, AUC50., AD, AR, XL, high_conservation, low_conservation) %>%
...
for (selected_tool in pa_per_tool_attribute_effect %>%
  pull(tool) %>%
  unique()) {
  print(selected_tool)
...
for (pa in performance_by_panelapp_table %>%
  pull(panelapp) %>%
  unique()) {
  print(pa)
  panel_table <- proc_data %>%
    mutate(
      AR = var_sets$AR, AD = var_sets$AD, XL = var_sets$XL,
...
for (go_col in go_cols) {
  ...
  for (tool in tools_to_test) {
    min_count <- proc_data %>%
      filter(var_sets[[year_to_analyze]], !is.na(!!sym(tool))) %>%
      count(!!sym(go_col), clinvar_class) %>%
...
go_process_to_test <- go_process_counts %>%
  group_by(go_col) %>%
  summarize(all_tools_above_thresh = all(min_count > min_count_thresh)) %>%
  filter(all_tools_above_thresh) %>%
  pull(go_col)
...
go_func_to_test <- go_func_counts %>%
  group_by(go_col) %>%
  summarize(all_tools_above_thresh = all(min_count > min_count_thresh)) %>%
  filter(all_tools_above_thresh) %>%
  pull(go_col)
...
balanced_sample_size_per_tool <-
  ...
  count() %>%
  ungroup() %>%
  group_by(name, cons_cat) %>%
  summarize(balanced_n = min(n))
...
calibration_by_conservation_plot <-
  ...
  mutate(
    value_cat = cut(value, breaks = c(seq(0, 1, 0.1)), labels = seq(0.05, 0.95, 0.1)),
    name = stringr::str_replace(name, "_score.+", "") %>% toupper()
  ) %>%
  group_by(name, value_cat, cons_cat) %>%
  count(clinvar_class) %>%
  mutate(
    value_cat = as.numeric(as.character(value_cat)),
    total = sum(n),
    rate = n / sum(n)
  ) %>%
...
calibration_by_conservation_plot
ggsave(
  filename = glue("{analysis_folder_name}/calibration_by_conservation_plot.{Sys.Date()}.png"),
...
z <-
  roc_metrics_table %>%
  filter(sub_var_set == "all", missing_option == "no_missing") %>%
  select(tool, main_var_set, bal_accuracy) %>%
  pivot_wider(names_from = main_var_set, values_from = bal_accuracy) %>%
  rowwise() %>%
  mutate(
...
proc_data %>%
  filter(main_var_sets[["confident_2020"]], sub_var_sets[["all"]]) %>%
...
roc_metrics_table %>%
  filter(n > 200) %>%
  filter(grepl("AD|AR", sub_var_set)) %>%
  filter(class_balance == "balanced_class") %>%
  filter(main_var_set == "confident_new") %>%
...
roc_metrics_table %>%
  filter(n > 50) %>%
  filter(grepl("af_cat", sub_var_set)) %>%
  filter(class_balance == "balanced_class") %>%
  filter(main_var_set == "confident_new") %>%
