...
- review the panelapp analysis - i think all the variants should either be TRUE or FALSE for each panel (currently there are NAs)
...
overall_performance_full_analysis_res$posterior_prob%>%
  filter(filter1==year_to_analyze)%>%
  mutate(tool=stringr::str_replace(tool,'_score.+','')%>%toupper())%>%
  ggplot(aes(x=tool,color=criteria,y=score,size=sensitivity))+
  geom_point(shape=3,stroke=2)+
  PrettyCols::scale_color_pretty_d(palette = 'Lively',scale_name='Lively')+
  facet_wrap(tool~.,scales='free')+
  theme_minimal()+theme(legend.position = 'top')+
  labs(x='Tool',y='Score',color='Criteria')
overall_performance_thresholds_plot
```
...
mismatchoe_score_thresholds%>%
  ...
  geom_line()+
  #geom_point(shape=3,size=2.5,stroke=2)+
  PrettyCols::scale_color_pretty_d(palette = 'Lively',scale_name='Lively')+
  facet_wrap(tool~.,scales='free_y')+
  theme_minimal()+theme(legend.position = 'top')+
...
pa_per_tool_all_comps<-pa_per_tool_all_comps%>%
  mutate(perf_diff=panel_auc-estimate)
