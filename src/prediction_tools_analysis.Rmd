...
analysis_folder_name<-'./output/prediction_tools_analysis.2024-09-04'
```
...
overall_performance_full_analysis_res<-calculate_roc_metrics(proc_data,
                                                             ...
                                                             converted_scores = converted_scores,
                                                             calculate_posterior_probs = TRUE,bw_method = 'ucv')
...
# collect features per panel
pa_attributes_table<-NULL
for (panel in performance_by_panelapp_table%>%pull(panelapp)%>%unique()){
  selected_panel<-glue('panelapp:{stringr::str_replace_all(panel," ",".")}')
  panel_table<-proc_data%>%
    ...
    summarize(n=n(),
              ...
              plp=sum(clinvar_class=='P/LP'),
              blb=sum(clinvar_class=='B/LB'))%>%
    filter(!!sym(selected_panel))
  pa_attributes_table<-pa_attributes_table%>%bind_rows(
    data.frame(panelapp_original=panel,panel_table%>%select(-selected_panel))
  )
}
...
for (pa in performance_by_panelapp_table%>%pull(panelapp)%>%unique()){
  ...
  average_overall_auc<-pa_comp_table%>%filter(panelapp=='overall_performance')%>%pull(AUC50.)%>%mean()
  average_pa_auc<-pa_comp_table%>%filter(panelapp!='overall_performance')%>%pull(AUC50.)%>%mean()
  pa_all_comps<-pa_all_comps%>%bind_rows(
    data.frame(panelapp=pa,average_overall_auc,average_pa_auc,data.frame(t(model_res$tTable[2,])),class_balance)
  )
}
...
library(ggrepel)
pa_all_comps<-pa_all_comps%>%mutate(panelapp_original=panelapp,
...
panelapp_overall_performance_diff_plot<-
 ...
 # geom_point(data=pa_all_comps%>%filter(p.value<0.01),color='blue')+
  ...
  geom_point(data=pa_all_comps%>%filter(p.value<0.01),color='darkorange')+
...
pa_per_tool_all_comps<-NULL
pa_per_tool_analysis_table<-
  performance_by_panelapp_table%>%
  filter(tool%in%top_performing_tools)
for (pa in pa_per_tool_analysis_table%>%pull(panelapp)%>%unique()){
  print(pa)
  for (selected_tool in pa_per_tool_analysis_table%>%pull(tool)%>%unique()){
    pa_tool_comp_table<-pa_per_tool_analysis_table%>%
      filter(tool==selected_tool)%>%
      mutate(panelapp=ifelse(panelapp==pa,pa,'other'))
    # ttest <- t.test(pa_tool_comp_table%>%filter(panelapp=='other')%>%pull(AUC50.), 
    #                 mu = pa_tool_comp_table%>%filter(panelapp==pa)%>%pull(AUC50.))
    # ttest_res<-tidy(ttest)
    tool_panel_auc<-pa_tool_comp_table%>%filter(panelapp==pa)%>%pull(AUC50.)
    tool_other_auc<-pa_tool_comp_table%>%filter(panelapp=='other')%>%pull(AUC50.)
    mean_samples <- mean(tool_other_auc)
    sd_samples <- sd(tool_other_auc)
    # Step 2: Calculate the z-score of sampleA
    z_score <- (tool_panel_auc - mean_samples) / sd_samples
    # Step 3: Calculate the probability of observing a value less than or equal to sampleA
    # Assuming normality, we can use the pnorm function
    probability <- min(1-pnorm(z_score),pnorm(z_score))
    # ecdf_fun <- ecdf(pa_tool_comp_table%>%filter(panelapp=='other')%>%pull(AUC50.))
    # # Probability that a value drawn from 'other_samples' is <= sampleA
    # probability <- min(1-ecdf_fun(tool_panel_auc),ecdf_fun(tool_panel_auc))
    
    pa_per_tool_all_comps<-pa_per_tool_all_comps%>%bind_rows(
      data.frame(panelapp=pa,tool=selected_tool,panel_auc=pa_tool_comp_table%>%filter(panelapp==pa)%>%pull(AUC50.),
                 p.value=probability)
    )
  }
}
pa_per_tool_all_comps$p.adjust<-p.adjust(pa_per_tool_all_comps$p.value,method='fdr')
pa_per_tool_all_comps<-pa_per_tool_all_comps%>%
  group_by(tool)%>%
  mutate(perf_diff=panel_auc-mean(panel_auc))%>%ungroup()

pa_per_tool_all_comps<-pa_per_tool_all_comps%>%bind_rows(
  pa_per_tool_all_comps%>%group_by(tool)%>%summarize(panelapp='overall',panel_auc=mean(panel_auc))
)%>%
    mutate(panelapp=relevel(factor(panelapp), ref = "overall"))

pa_lme_res<-lme(panel_auc ~ panelapp,random=~1 | tool, data = pa_per_tool_all_comps)
a<-summary(pa_lme_res)
z<-a$tTable

panelapp_performance_plot_per_panel<-
pa_per_tool_all_comps%>%
  #filter(p.adjust<0.05)%>%
  mutate(panelapp=stringr::str_replace_all(panelapp,'\\.',' '))%>%
  ggplot(aes(x = reorder(panelapp, perf_diff, FUN = median), y = perf_diff))+
  geom_boxplot(outlier.size = 0.5,aes(fill=..middle..),alpha=0.5)+
  geom_hline(yintercept = 0,linetype=2)+
  coord_flip()+
  theme_minimal()+
...
pa_per_tool_all_comps%>%
  group_by(panelapp)%>%
  summarize(sig_better=sum(ifelse(p.adjust<0.05 & perf_diff>0,1,0))/n(),
            sig_worse=sum(ifelse(p.adjust<0.05 & perf_diff<0,1,0))/n(),
            total=n())%>%
  ungroup()

# compare features for variants in panels that have differing performance
pa_better_worse_comp<-pa_attributes_table%>%
  mutate(better_worse=ifelse(feature_comp%>%filter(panelapp==panelapp_original)%>%pull(sig_better)>0.6,'better',
                             ifelse(feature_comp%>%filter(panelapp==panelapp_original)%>%pull(sig_worse)>0.6,'worse','same')))%>%
  rowwise()%>%
  mutate(avg_diff=mean(pa_per_tool_all_comps%>%filter(panelapp==panelapp_original)%>%pull(perf_diff)))%>%
  ungroup()%>%
  mutate(avg_diff_cat=cut(avg_diff,3,labels = c('AUC decrease','AUC same','AUC increase')),
         plp_rate=plp/(plp+blb))

library(gtsummary)
pa_better_worse_comp%>%
  select(-c(panelapp_original,better_worse,plp,blb,avg_diff))%>%
  tbl_summary(by = avg_diff_cat) %>%
  add_p()%>%
  as_tibble()%>%
